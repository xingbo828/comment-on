...
import { StepContainer, StepLabel, StepSeperatorWrapper, StepHighLightBar } from './Styled';

const Step = ({ index, title, status, onStepClick, ...rest }) => {
  const renderContent = (title) => {
      return (
        <StepLabel>
          <StepSeperatorWrapper>{status === 'completed' ? <Icon icon="check" />: `${(index + 1)}.`}</StepSeperatorWrapper>{title}
        </StepLabel>
      );
  }
